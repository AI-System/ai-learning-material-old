...
4. 使用字符串连接和 `len` 函数计算某些电影明星的实际完整姓名的长度
...
5. 我们刚刚使用函数 `len` 计算出字符串的长度。当我们向其提供整数 `835` 而不是字符串时，函数 `len` 会返回什么？
...
4. 以下对象是什么类型：`len("my_string")`？

  ```
  int
  ```

5. 以下对象是什么类型：`"hippo" *12`？
...
1. 对浮点型对象调用 `islower` 等方法会发生什么？例如 `13.37.islower()`
...
1. 请使用列表索引根据整型变量 `month` 判断特定月份有多少天，并将该值存储在整型变量 `num_days` 中
...
2. 请使用列表切片记法从此列表中选择列表中的最后三个元素。提示：切片可以使用负索引！
